...
data "azurerm_subnet" "this" {
  ...
  resource_group_name  = element(var.allowed_networks, count.index)["networking_resource_group"] != null ? element(var.allowed_networks, count.index)["networking_resource_group"] : (var.resource_group_name : data.azurerm_resource_group.this.0.name)
}

data "azurerm_storage_account" "this" {
  count               = (var.auditing_storage_account_name != null) ? 1 : 0
  name                = var.auditing_storage_account_name
  resource_group_name = var.resource_group_name : data.azurerm_resource_group.this.0.name
}
...
locals {
  tags                       = merge(var.azuresql_additional_tags, (lookup(data.terraform_remote_state.resourcegroup.outputs.resource_group_tags_map, var.resource_group_name) : data.azurerm_resource_group.this.0.tags))
...
resource "azurerm_mssql_server_extended_auditing_policy" "primary" {
  ...
  storage_endpoint  = lookup(data.terraform_remote_state.storage.outputs.primary_blob_endpoints_map, var.auditing_storage_account_name) : data.azurerm_storage_account.this.0.primary_blob_endpoint
...
resource "azurerm_role_assignment" "secondary" {
  ...
  scope                = lookup(data.terraform_remote_state.storage.outputs.sa_ids_map, var.auditing_storage_account_name) : data.azurerm_storage_account.this.id
  role_definition_name = "Storage Blob Data Contributor"
