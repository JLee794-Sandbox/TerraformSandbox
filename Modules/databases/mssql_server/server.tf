resource "azurerm_mssql_server" "mssql" {
  ...
  dynamic "azuread_administrator" {
    for_each = lookup(var.settings, "azuread_administrator", {}) != {} ? [var.settings.azuread_administrator] : []
...
resource "azurerm_key_vault_secret" "sql_admin_password" {
  ...
  expiration_date = can(var.settings.keyvault_secret_expiration_date) ? var.settings.keyvault_secret_expiration_date : timeadd(timestamp(), "4320h")
