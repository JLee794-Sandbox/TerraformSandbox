data "azurerm_storage_account" "backup_storage_account" {
  count = lookup(var.settings, "backup", null) != null ? 1 : 0
...
data "azurerm_storage_account_blob_container_sas" "logs" {
  count = lookup(var.settings, "logs", null) != null ? 1 : 0

  connection_string = data.azurerm_storage_account.backup_storage_account.0.primary_connection_string
  container_name    = try(local.logs_storage_account.containers[var.settings.logs.container_key].name, null)
  https_only        = true
...
data "azurerm_storage_account_blob_container_sas" "http_logs" {
  count = lookup(var.settings, "logs", null) != null ? (lookup(var.settings.logs, "http_logs", null) != null ? 1 : 0) : 0

  connection_string = data.azurerm_storage_account.backup_storage_account.0.primary_connection_string
  container_name    = try(local.http_logs_storage_account.containers[var.settings.logs.http_logs.container_key].name, null)
  https_only        = true
