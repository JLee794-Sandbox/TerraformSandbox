...
locals {
  ...
  backup_storage_account = can(var.settings.backup) ? try(var.storage_accounts[var.settings.backup.storage_account_key], null) : null
  ...
  logs_storage_account = can(var.settings.logs) ? try(var.storage_accounts[var.settings.logs.storage_account_key]) : null
  logs_sas_url         = can(var.settings.backup) ? "${local.logs_storage_account.primary_blob_endpoint}${local.logs_storage_account.containers[var.settings.logs.container_key].name}${data.azurerm_storage_account_blob_container_sas.logs[0].sas}" : null

  http_logs_storage_account = can(var.settings.logs.http_logs) ? try(var.storage_accounts[var.settings.logs.http_logs.storage_account_key]) : null
  ...
  ## Ommitting the below code to remove the landing zone key based access to attributes for the caf-terraform module wrapper
